...
sealed class Contact {
    ...
    companion object {
        ...
        fun vntContact(name: String): BaresipContact? {
            for (c in BaresipService.vntContacts.value)
                if (c.name == name)
                    return c
            return null
        }
        ...
        fun contactsUpdate() {
            ...
            if (BaresipService.contactsMode != "vnt")
                for (c in BaresipService.androidContacts.value)
        ...
        fun addBaresipContact(contact: BaresipContact) {
            BaresipService.vntContacts.value += contact
            saveBaresipContacts()
            contactsUpdate()
        }

        fun updateBaresipContact(id: Long, contact: BaresipContact) {
            val updatedContacts = BaresipService.vntContacts.value.toMutableList()
            updatedContacts.removeIf { it.id == id }
            updatedContacts.add(contact)
            BaresipService.vntContacts.value = updatedContacts.toList()
            saveBaresipContacts()
            contactsUpdate()
        }

        fun removeBaresipContact(contact: BaresipContact) {
            val removed = BaresipService.vntContacts.value.toMutableList()
            removed.removeIf { it.id == contact.id }
            BaresipService.vntContacts.value = removed.toList()
            saveBaresipContacts()
            contactsUpdate()
        }
