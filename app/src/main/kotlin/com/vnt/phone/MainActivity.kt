...
class MainActivity : ComponentActivity() {
    ...
    private lateinit var vntService: Intent
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
            ...
            if (event != null && BaresipService.serviceEvents.isNotEmpty()) {
                ...
                if (taskId != -1) {
                    if (first.event == "started" && !initialized)
                        // Android has restarted vnt when permission has been denied in app settings
                        recreate()
                    else {
        ...
        val restartApp = {
            Log.i(TAG, "Restarting vnt")
            window.setFlags(WindowManager.LayoutParams.FLAG_NOT_TOUCHABLE,
                WindowManager.LayoutParams.FLAG_NOT_TOUCHABLE)
            if (BaresipService.isServiceRunning) {
                restart = true
                vntService.action = "Stop"
                ContextCompat.startForegroundService(this, vntService)
            } else {
                finishAndRemoveTask()
        ...
        vntService = Intent(this@MainActivity, BaresipService::class.java)
