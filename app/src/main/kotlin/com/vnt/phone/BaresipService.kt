...
import com.vnt.phone.Utils.e164Uri
import com.vnt.phone.Utils.toCircle
...
class BaresipService: Service() {
    ...
    override fun onCreate() {
        ...
        intent = Intent("com.vnt.phone.EVENT")
        intent.setPackage("com.vnt.phone")
        ...
        proximityWakeLock = pm.newWakeLock(PowerManager.PROXIMITY_SCREEN_OFF_WAKE_LOCK,
                "com.vnt.phone:proximity_wakelock")

        wifiLock = if (VERSION.SDK_INT < 29)
            @Suppress("DEPRECATION")
            wm.createWifiLock(WifiManager.WIFI_MODE_FULL_HIGH_PERF, "VNT")
        else
            wm.createWifiLock(WifiManager.WIFI_MODE_FULL_LOW_LATENCY, "VNT")
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        when (action) {
            ...
            "Start" -> {
                ...
                if (!file.exists()) {
                    Log.i(TAG, "Creating vnt directory")
                    try {
                ...
                Thread {
                    vntStart(
                        ...
                        if (userAgent != "")
                            ...
                            "vnt v${BuildConfig.VERSION_NAME} " +
    ...
    override fun onDestroy() {
        ...
        instance = null
        if (isServiceRunning)
            sendBroadcast(Intent("com.vnt.phone.Restart"))
    }
    ...
    @Keep
    fun started() {
        Log.d(TAG, "Received 'started' from vnt")
        isNativeReady = true
        addMobileUserAgent()
        Api.net_debug()
        postServiceEvent(
    ...
    fun stopped(error: String) {
        Log.d(TAG, "Received 'stopped' from vnt with start error '$error'")
        isNativeReady = false
        quitTimer.cancel()
        cleanService()
        isServiceRunning = false
    ...
    private fun isStarredContact(callerNumber: String): Boolean {
        if (contactsMode == "vnt" || callerNumber.isBlank())
            return false
    ...
    external fun vntStop(force: Boolean)
    ...
    companion object {
        ...
        val vntContacts = mutableStateOf(emptyList<Contact.BaresipContact>())
        ...
        var contactsMode by mutableStateOf("vnt")
        var addressFamily = ""
        ...
        internal const val KEY_TEXT_REPLY = "key_text_reply_vnt"
        private const val PHONE_ACCOUNT_ID = "vnt_phone_account"
